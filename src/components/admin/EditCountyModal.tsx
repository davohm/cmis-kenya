...
import { County } from '../../hooks/useCountyManagement';

interface EditCountyModalProps {
  isOpen: boolean;
  onClose: () => void;
  county: County | null;
  onSuccess: () => void;
...
export default function EditCountyModal({
  isOpen,
  onClose,
  county,
  onSuccess
}: EditCountyModalProps) {
  const { updateCounty, loading } = useCountyCRUD();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMessage(`Successfully updated county: ${formData.name}`);
      ...
      setErrorMessage(err instanceof Error ? err.message : 'Failed to update county');
    }
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden">
        <div className="flex items-center justify-between p-6 border-b border-gray-200 bg-gradient-to-r from-red-600 to-red-700">
          <div>
            <h2 className="text-2xl font-bold text-white">Edit County</h2>
            <p className="text-red-100 text-sm mt-1">Update county information</p>
          </div>
          <button
            onClick={onClose}
